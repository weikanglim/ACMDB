...
if(Input::exists('get')){
	...
	if (Token::check ( Input::get ( Config::get ( 'session/token_name' ) ) )) {
		...
		if (Input::get ( 'edit' )) {
			...
			$record = $dbo->get ( 'transactions', array (
					"{$primary_key}",
					'=',
					"{$primary_value}" 
			) )->first ();
...
<div>
	<form action="">
		...
		<?php 
		if(!$edit){
			echo Format::adminView($records, $headers, 'balance.php');
		}else{
			?>
			
			<form action="" method="post">
			<?php 
			echo Format::record($record, $mappedFields, $primary_key , true);
			?>
			<input type="hidden" name="fields" value="<?php echo implode(":", $fields);?>">
			<input type="hidden" name="primary" value="<?php echo "{$primary_value}";?>">

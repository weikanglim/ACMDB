...
$primary_key = 'gid';
$dbo = DB::getInstance()->query("SELECT * FROM {$table} AS SG WHERE SG.GID IN (SELECT G.GID FROM SIGGROUPS AS G WHERE G.UID = ?)", array($uid));
$edit = false;
$error = "";
...
if(Input::exists('post')){
	...
	if ($validation->passed ()) {
		...
		$fieldAndValue = array();
		$otherField = array();
		$otherField['title'] = Input::get('group_name'); 
		foreach($fields as $field){
			if($field !== 'group_name' && $field !== $primary_key){
